...
const refreshMaterialUsageReportForm = () => {
    ...
    let minEndDate = currentDateForMinEDate.setDate(currentDateForMinEDate.getDate() - 29);
...
const generatematerialusagereport = () => {
    ...
    if (error == '') {
        let dataListReport = ajaxGetRequestMapping("/materialusagereportbydate/" + reportmonthlymaterialusage.sdate + "/" + reportmonthlymaterialusage.edate);
    ...
    if (oblist.length > 0) {
        if (oblist.length > 100) {
            ...
            if (confirmDisplay) {
                ...
                fillDataIntoSingleBarChart("reportChart", xAxesRaw, yAxes, labels, "Daily Material Usage Report");
            } else {
